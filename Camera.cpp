                                      ...
                                      KeyCode moveForward, KeyCode moveBackward, KeyCode moveLeft, KeyCode moveRight)
{
	...
	if (KeyHeld(Key_Down))
	{
		mRotation.x += ROTATION_SPEED * frameTime /2; // Use of frameTime to ensure same speed on different machines
	}
	if (KeyHeld(Key_Up))
	{
		mRotation.x -= ROTATION_SPEED * frameTime /2;
	}
	if (KeyHeld(Key_Right))
	{
		mRotation.y += ROTATION_SPEED * frameTime /4.5;
	}
	if (KeyHeld(Key_Left))
	{
		mRotation.y -= ROTATION_SPEED * frameTime /4.5;
	...
	if (KeyHeld(Key_D))
	{
		mPosition.x += MOVEMENT_SPEED * frameTime/2 * mWorldMatrix.e00; // See comments on local movement in UpdateCube code above
		mPosition.y += MOVEMENT_SPEED * frameTime/2 * mWorldMatrix.e01;
		mPosition.z += MOVEMENT_SPEED * frameTime / 2 * mWorldMatrix.e02;
	}
	if (KeyHeld(Key_A))
	{
		mPosition.x -= MOVEMENT_SPEED * frameTime / 2 * mWorldMatrix.e00;
		mPosition.y -= MOVEMENT_SPEED * frameTime / 2 * mWorldMatrix.e01;
		mPosition.z -= MOVEMENT_SPEED * frameTime / 2 * mWorldMatrix.e02;
	}
	if (KeyHeld(Key_W))
	{
		mPosition.x += MOVEMENT_SPEED * frameTime / 2 * mWorldMatrix.e20;
		mPosition.y += MOVEMENT_SPEED * frameTime / 2 * mWorldMatrix.e21;
		mPosition.z += MOVEMENT_SPEED * frameTime / 2 * mWorldMatrix.e22;
	}
	if (KeyHeld(Key_S))
	{
		mPosition.x -= MOVEMENT_SPEED * frameTime / 2 * mWorldMatrix.e20;
